...
function BidViewHandle(){
	this.form = new Form("BidForm");
	this._attach_list = {}, this.aptitudesFile = {arr : [], index : 0}, this.iFile = {arr : [], index : 0};
	this.loadData = function(data){
		$.post(CTX + "/bid/getBidViewObjBySid", { sid : data.sid }, function(result) {
			if(result.status){
				instance._attach_list = result.result;
				instance.form.setData(result.result.mapdata);
				var ibiddingtype = "";
				if(result.result.mapdata != null && result.result.mapdata != ""){
					var itype= Globle.constant.iBiddingType;
					var text = "";
					for(var i = 0 ; i < itype.length; i++){
						if(result.result.mapdata.ibiddingtype == itype[i].value) ibiddingtype = itype[i].text;
					}
				}
				search.get("ibiddingtype").setValue(ibiddingtype);
...
var instance = new BidViewHandle();

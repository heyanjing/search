search.parse();
var tdg = search.get("areasgrid");
var areas = search.get("areas");

//新增
function add(){
	var data = {
			url : CTX +"/areas/updateAreasPage",
			title : "新增",
			width : 450,
			height : 350,
...
function onState(state,sid) {
	...
	if(sid){
		...
		top.search.confirm({ content : "确定要"+str+"吗？", funl : function() {
			$.post(CTX +"/areas/updateState", { id : ids,state : state}, function() {
				tdg.reload();
			})
		} })
	}else{
...
function delData(sid){
	...
	if(sid){
		...
		top.search.confirm({ content : "确定要删除吗？", funl : function() {
			$.post(CTX +"/areas/updateState", { id : ids,state : 99}, function() {
				tdg.reload();
			})
		} })
	}else{
...
function queryDetail(params) {
	...
	top.search.popDialog({ url : CTX + "/areas/divView?sid="+sid, title : "查看区域", width : 450, height : 250,
		onload : function(window) {
...
$(function(){
	tdg.url = CTX + "/areas/getAreaData";
	tdg.load({state : __state});
	
	areas.url = CTX + "/areas/findAreas";
	areas.load();
});
...
function searchGrid() {
	...
	var areas = search.get("areas").getValue();

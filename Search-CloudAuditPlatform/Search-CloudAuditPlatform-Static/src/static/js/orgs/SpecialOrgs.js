...
function OrgsMgrHandle(){
	grid.url = CTX + "/orgs/qsos";
//	var str = null;
//	if(sname.getValue()!=null&&sname.getValue()!=""){
//		str = getstr(sname.getValue());
//	}
	grid.load({state : __state,sname:sname.getValue()});
	$.ajax({
		url : CTX+"/orgs/gui",
...
function add(){
	...
	var data = {
		...
		onload : function(window){
			window.setData({sid: sid,action:"add"});
		},
		ondestroy : function(){
			initalQuery();
		}
...
function authorization(){
	if(__parentareaid==null&&__orgid!=null){
		var data = {
				url : CTX+"/orgs/gabso",
				title : "授权",
				width : 800,
		...
		if(select.record.itype==Globle.constant.SpecialOrgType[2].value){
			search.warn({content:"政府机构不能授权!"});
			return;
		}
		var data = {
				url : CTX+"/orgs/gabsobai?areaid="+select.record.sareaid,
				title : "授权",
				width : 800,
				height : 550,
				onload : function(window){
					window.setData({areaid: select.record.sareaid,orgid: select.record.sid});
				},
				ondestroy : function(){
					initalQuery();
				}
...
function addmanager(){
	...
	if(select.record.itype==Globle.constant.SpecialOrgType[2].value){
		search.warn({content:"政府机构不能新增用户!"});
		return;
	}
	var data = {
		url : CTX+"/orgs/gamso?areaid="+select.record.sareaid,
		title : "新增用户",
		width : 800,
		height : 550,
		onload : function(window){
			window.setData({sid: select.record.sid,userid:select.record.userid,areaid:select.record.sareaid});
		},
		ondestroy : function(){
			initalQuery();
		}

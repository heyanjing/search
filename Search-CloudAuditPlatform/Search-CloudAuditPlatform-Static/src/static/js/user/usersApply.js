...
$(function() {
	...
	if(type != 1){//管理员
		grid.url = CTX + "/user/findPageUsersIstate";
		grid.load();
	}else{
...
// 审核
function examineUser() {
	var selects = grid.getSelecteds();
	if(selects.length == 0){
		top.search.error({ content : "请选择一条数据!" });
		return;
	}
	top.search.popDialog({ url : CTX + "/user/getExaminePage", title : "审核", width : 800, height : 550, onload : function(window) {
		window.setData({ action : "edit", sid : selects[0].record.sid,osid : selects[0].record.osid });

...
//新增
function add(){
	$.ajax({ url : CTX + "/processdesigns/checkWhetherOperableProcessDesigns",data : {sfunctionid : id},
		 success : function(result) {
			 console.log(result);
			 if(result.status){
				 var data = {
							url : CTX +"/Planlibs/goUpdatePlanlibsPage?id="+id+"&sid="+result.result.sid+"&type="+type,
							title : "创建",
							width : 800,
							height : 600,
							onload : function(window){
								window.setData({ action : "add",funcid : __funcid});
							},
							ondestroy : function(){
								reload();
							}
...
function edit(params){
	if(params){
		...
		if(select){
			...
			top.search.popDialog({ 
				url : CTX +"/Planlibs/goUpdatePlanlibsPage?id="+id+"&type="+type,
				title : "编辑", 
				width : 800,
				height : 600, 
				onload : function(window) {
				window.setData({ action : "edit", id : select.record.sid,sProcessInstanceId : select.record.sprocessinstanceid,type:type,funcid : __funcid});
			}, ondestroy : function(result) {
					reload();
			} });
		}else{
...
function Submit(){
	...
	if(select){
		...
		top.search.popDialog({ 
			url : CTX +"/Planlibs/goUpdatePlanlibsPage?id="+id+"&type="+type+"&orgid="+select.record.sorgid,
			title : "编辑", 
			width : 800,
			height : 600, 
			onload : function(window) {
			window.setData({ action : "edit", id : select.record.sid,sProcessInstanceId : select.record.sprocessinstanceid,sub : 1,type:type,orgid:select.record.sorgid,funcid : __funcid});
		}, ondestroy : function(result) {
				reload();
		} });
	}else{
...
function queryDetail(params) {
	...
	top.search.popDialog({ url : CTX + "/Planlibs/goplanView", title : "详情", width : 800,
		height : 600,
		onload : function(window) {
			window.setData({ action : "edit",id : sid,type:type,sub : 1,funcid : __funcid });
		}, ondestroy : function(result) {
...
$(function(){
	tdg.url = CTX + "/Planlibs/getPlanlibsData";
	tdg.load({state : __state,type:type});
	
	orgid.url = CTX + "/orgs/orgaudit";
	orgid.load();
});
